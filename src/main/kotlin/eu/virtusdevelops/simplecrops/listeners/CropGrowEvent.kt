    ...
    fun onCropGrow(event: BlockGrowEvent){
        ...
        if(block.type == Material.AIR){
            ...
            if(base.type == Material.CACTUS){
                ...
                if(stem != null){
                    val stemBlock = stem.block
                    val cropLocation = CropLocation(stemBlock.x, stemBlock.y, stemBlock.z, stemBlock.world.name)
                    val crop = cropStorage.crops[cropLocation.toString()]
                    if(crop != null) {
                        event.isCancelled = true
                        ...
                        if (player != null) {
                            ...
                            particles.growEffect(player, stemBlock.location)
                        }
                        // set stem direction
                        stem.update()
                    }
