                    ...
                    if(item.type.toString().contains("HOE")){ // if its hoe then do fastharvest stuff.
                        if(nbtUtil.nbt.hasValueString(item, "hoeID")){
                            if(player.hasPermission("simplecrops.fastharvest")) {
                                ...
                                if (uses == 1) {
                                    PlayerUtil.breakAnimation(player, item)
                                    player.inventory.setItemInMainHand(null)
                                }
                                // HOE HANDLER 3x3, 5x5 ....
                                val blocks = BlockUtil.getSquare(block, size / 2)
                                var update = false
                                    ...
                                    if (CropUtil.isCrop(sblock) && !CropUtil.isMultiBlock(sblock)) {
                                        if (CropUtil.isFullyGrown(sblock)) {
                                            ...
                                            if (newcrop != null) {
                                                ...
                                                //player.spawnParticle(Particle.CLOUD, block.location, 2)
                                                newcrop.bonemeal = 0
                                                player.spawnParticle(Particle.CLOUD, sblock.x.toDouble() + 0.5, sblock.y.toDouble() + 0.5, sblock.z.toDouble(), 5, 0.01, 0.0, 0.01, 0.02)
                            ...
                            if(CropUtil.isFullyGrown(block)) {
                                if (player.hasPermission("simplecrops.fastharvest")) {
                                    ...
                                    crop.bonemeal = 0
                                    player.spawnParticle(Particle.CLOUD, block.x.toDouble() + 0.5, block.y.toDouble() + 0.5, block.z.toDouble(), 5, 0.01, 0.0, 0.01, 0.02)
                        ...
                        if(nbtUtil.nbt.hasValueString(item, "hoeID")){
                            ...
                            blocks.forEach { blocke ->
                                if(blocke.type == Material.GRASS_BLOCK || blocke.type == Material.DIRT) {
                                    if (blocke.getRelative(BlockFace.UP).isPassable) {

                                        Bukkit.getScheduler().runTaskLater(plugin, Runnable{
                                            blocke.setType(Material.FARMLAND, true)
                                            blocke.getRelative(BlockFace.UP).breakNaturally()
                                        }, (10L..20L).random())
